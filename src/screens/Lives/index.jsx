...
export default function Lives() {
  ...
  return (
    <div className="container mx-auto py-8 px-4 space-y-12">
          ...
          {streams.map((stream) => (
            <Link to={`/stream/${stream.id}`} key={stream.id} className="no-underline">
              <Card className="hover:shadow-lg transition-shadow duration-300 h-full">
                ...
                <CardFooter className="text-sm text-gray-500 flex items-center mt-auto space-x-2">
                  <Users className="w-4 h-4" />
                  <span>{stream.category}</span>
                  <Bell className="w-4 h-4 ml-auto cursor-pointer hover:text-gray-700" title="Get notified" />
                </CardFooter>
              </Card>
            </Link>
          ))}
      ...
      <section>
        <h2 className="text-3xl font-semibold mb-4 text-gray-800">Upcoming Streams</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {upcomingStreams.map((stream) => (
            <Card key={stream.id} className="hover:shadow-lg transition-shadow duration-300 h-full">
              <CardHeader className="relative p-0">
                <img src={stream.thumbnail} alt={stream.title} className="w-full h-48 object-cover rounded-t-lg" />
                <Badge className="absolute top-2 left-2 bg-yellow-600 flex items-center">
      ...
      <section>
        <h2 className="text-3xl font-semibold mb-4 text-gray-800">Recent Posts</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post) => (
            <Card key={post.id} className="hover:shadow-lg transition-shadow duration-300 h-full">
              ...
              <CardFooter className="text-sm text-gray-500 flex items-center mt-auto space-x-4">
                <span>{post.date}</span>
      ...
      <section>
        <h2 className="text-3xl font-semibold mb-4 text-gray-800">New Post</h2>
        <form onSubmit={handleSubmit} className="p-4 border border-gray-300 rounded-lg bg-white shadow">
          <div className="mb-4">
          ...
          <div className="mb-4">
            <label htmlFor="authorName" className="block text-gray-700 font-semibold mb-2">Author Name</label>
            <input
              id="authorName"
              name="authorName"
              type="text"
          ...
          <div className="mb-4">
            <label htmlFor="authorAvatar" className="block text-gray-700 font-semibold mb-2">Author Avatar URL (Optional)</label>
            <input
              id="authorAvatar"
              name="authorAvatar"
              type="text"
